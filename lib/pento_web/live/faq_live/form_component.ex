defmodule PentoWeb.FAQLive.FormComponent do
  ...
  def render(assigns) do
    ...
    <div>
      ...
      >
        ...
        <.input field={@form[:vote_count]} type="number" label="Vote count" step="any" />
        <:actions>
  ...
  def update(%{faq: faq} = assigns, socket) do
    changeset = Extention.change_faq(faq)
  ...
  def handle_event("validate", %{"faq" => faq_params}, socket) do
    changeset =
      socket.assigns.faq
      |> Extention.change_faq(faq_params)
  ...
  defp save_faq(socket, :edit, faq_params) do
    case Extention.update_faq(socket.assigns.faq, faq_params) do
      {:ok, faq} ->
  ...
  defp save_faq(socket, :new, faq_params) do
    case Extention.create_faq(faq_params) do
      {:ok, faq} ->
        notify_parent({:saved, faq})
